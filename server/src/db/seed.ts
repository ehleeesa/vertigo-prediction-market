...
async function insertUsers() {
  console.log(`Creating admin and ${USER_COUNT} users...`);
  ...
  const [createdAdmin] = await db.insert(schema.usersTable).values(adminUser).returning();
  insertedUsers.push(createdAdmin);
